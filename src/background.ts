    ...
    if (alarm.name === "updateSalahAlarmsAfterMidnight") {
      ...
      for (const team of storage.get("slackTeams")) {
        if (team && "access_token" in team) {
          ...
          await slack.setUserStatus(
            {
              statusEmoji: lastSlackProfileStatus.profile.status_emoji.includes(
                "mosque"
              )
                ? ""
          ...
          if (
            ...
            await slack.setSnooze(
              lastSlackDndSnoozeSettings.snooze_remaining / 60 -
      ...
      for (const team of storage.get("slackTeams")) {
        if (team && "access_token" in team) {
          ...
          await storage.saveSlackLastProfileStatus(
            team.access_token,
            team.team_id
          );
